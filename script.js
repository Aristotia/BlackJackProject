...
while (gameOver) {
  ...
  if (playerTotal < 21) {
    while (confirm("Voulez-vous tirer une nouvelle carte?")) {
      ...
      playerTotal += playerValue3;
      console.log(
        `Votre nouvelle carte est le ${playerValue3}.Votre total est de ${playerTotal}.`
      );
      if (playerTotal >= 21) {
        break;
      }
  ...
  if (dealerTotal < 17) {
    ...
    dealerTotal += dealerValue3;
    console.log(
      `La carte tirée vaut ${dealerValue3}, le total du dealer est maintenant de ${dealerTotal}.`
    );
  }
